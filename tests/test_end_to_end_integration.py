...
class TestEndToEndIntegration:
    ...
    async def test_all_tools_registered(self, mcp_server):
        ...
        for expected_tool in expected_tools:
            assert expected_tool in tool_names, "Tool '{expected_tool}' not registered"

        assert (
            len(tool_names) == 8
        ), "Expected 8 tools, found {len(tool_names)}: {tool_names}"
    ...
    async def test_get_technique_mitigations_tool_execution(self, mcp_server):
        ...
        # Execute the tool
        result, _ = await mcp_server.call_tool(
            "get_technique_mitigations", {"technique_id": "T1059"}
        )
    ...
    async def test_error_handling_integration(self, mcp_server):
        ...
        result, _ = await mcp_server.call_tool(
            "get_technique", {"technique_id": "INVALID"}
        )
        assert result is not None
        ...
        result, _ = await mcp_server.call_tool(
            "get_group_techniques", {"group_id": "INVALID"}
        )
        assert result is not None
        assert "not found" in result[0].text.lower()
        ...
        result, _ = await mcp_server.call_tool(
            "get_technique_mitigations", {"technique_id": "INVALID"}
        )
        assert result is not None
        assert "not found" in result[0].text.lower()
