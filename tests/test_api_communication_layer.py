...
class TestAPIEndpoints(AioHTTPTestCase):
    ...
    async def get_application(self):
        ...
        sample_data = {
            "techniques": [
                {"id": "T1055", "name": "Process Injection", "description": "Test technique"},
                {"id": "T1059", "name": "Command and Scripting Interpreter", "description": "Test technique 2"}
            ],
            "tactics": [
    ...
    async def test_techniques_endpoint_with_query(self):
        ...
        for technique in techniques:
            ...
            query = "process"
            self.assertTrue(
                query.lower() in technique["id"].lower() or
                query.lower() in technique["name"].lower() or
...
class TestAPIValidation:
    ...
    def test_tool_parameter_validation(self):
        ...
        assert self._validate_tool_params("get_technique", {"technique_id": "T1055"}) == True
        
        # Invalid parameters
        assert self._validate_tool_params("", {"query": "test"}) == False  # Empty tool name
        assert self._validate_tool_params("search_attack", None) == False  # Null parameters
        assert self._validate_tool_params("search_attack", "invalid") == False  # Non-object parameters
