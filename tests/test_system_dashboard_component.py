...
class TestSystemDashboardComponent:
    ...
    def test_components_js_file_exists(self, components_js_content):
        ...
        assert (
            "constructor(containerId)" in components_js_content
    ...
    def test_dashboard_template_structure(self, components_js_content):
        ...
        assert (
            "getDashboardTemplate()" in components_js_content
        ), "Should have getDashboardTemplate method"
        assert (
            "getLoadingTemplate()" in components_js_content
        ), "Should have getLoadingTemplate method"
        assert (
            "getErrorTemplate()" in components_js_content
        ), "Should have getErrorTemplate method"

        # Check for required HTML elements in templates
        assert (
            "dashboard-title" in components_js_content
        ), "Should include dashboard title"
        ...
        assert (
            "server-info" in components_js_content
        ), "Should include server info section"
    ...
    def test_dashboard_stat_elements(self, components_js_content):
        ...
        for element_id in stat_elements:
            assert (
                element_id in components_js_content
            ), f"Should include {element_id} element"

        # Check for server info elements
        server_info_elements = ["server-version", "data-source", "last-updated"]
    ...
    def test_dashboard_loading_state(self, components_js_content):
        ...
        assert (
            "renderLoadingState()" in components_js_content
        ), "Should have renderLoadingState method"
        assert (
            "getLoadingTemplate()" in components_js_content
        ), "Should have getLoadingTemplate method"
        assert (
            "loading-skeleton" in components_js_content
        ), "Should include loading skeleton elements"
        assert (
            "getSkeletonStatCard" in components_js_content
        ), "Should have skeleton stat card method"
    ...
    def test_dashboard_error_state(self, components_js_content):
        ...
        assert (
            "renderError()" in components_js_content
        ), "Should have renderError method"
        assert (
            "getErrorTemplate()" in components_js_content
        ), "Should have getErrorTemplate method"
        ...
        assert (
            "setupErrorEventListeners()" in components_js_content
        ), "Should have error event listeners"
    ...
    def test_dashboard_interaction_methods(self, components_js_content):
        ...
        assert (
            "setupEventListeners()" in components_js_content
        ), "Should have setupEventListeners method"
        assert (
            "handleStatCardHover" in components_js_content
        ), "Should have hover handler"
        assert (
            "handleStatCardLeave" in components_js_content
        ), "Should have leave handler"
        assert (
            "handleStatCardClick" in components_js_content
    ...
    def test_dashboard_custom_events(self, components_js_content):
        ...
        assert (
            "statCardClick" in components_js_content
        ), "Should emit statCardClick event"
    ...
    def test_dashboard_utility_methods(self, components_js_content):
        ...
        assert (
            "getSystemInfo()" in components_js_content
        ), "Should have getSystemInfo method"
        assert (
            "isLoadingData()" in components_js_content
        ), "Should have isLoadingData method"
        assert (
            "hasErrorState()" in components_js_content
        ), "Should have hasErrorState method"
    ...
    def test_dashboard_animation_methods(self, components_js_content):
        ...
        assert (
            "animateStatUpdate" in components_js_content
        ), "Should have stat animation method"
        assert (
            "requestAnimationFrame" in components_js_content
        ), "Should use requestAnimationFrame"
        assert (
            "toLocaleString" in components_js_content
    ...
    def test_dashboard_accessibility_features(self, components_js_content):
        ...
        assert (
            "dashboard-title" in components_js_content
        ), "Should have proper title class"
        assert (
            "bi bi-shield-check" in components_js_content
        ), "Should have icon for visual context"
    ...
    def test_dashboard_error_handling(self, components_js_content):
        ...
        assert (
            "this.hasError = true" in components_js_content
        ), "Should track error state"
        assert (
            "this.errorMessage" in components_js_content
    ...
    def test_dashboard_data_handling(self, components_js_content):
        ...
        assert (
            "|| 0" in components_js_content
        ), "Should have fallback values for missing data"
        assert (
            "|| 'Unknown'" in components_js_content
        ), "Should have fallback text for missing info"
        assert (
            "stats.techniques_count || 0" in components_js_content
        ), "Should handle missing technique count"
        assert (
            "serverInfo.version || 'Unknown'" in components_js_content
    ...
    def test_dashboard_api_integration(self, components_js_content):
        ...
        # Check for API usage
        assert (
            "api.getSystemInfo()" in components_js_content
        ), "Should call API for system info"
        assert (
            "await api.getSystemInfo()" in components_js_content
        ), "Should use async/await"
        assert (
            "this.systemInfo = await" in components_js_content
        ), "Should store API response"
    ...
    def test_dashboard_css_classes(self, components_js_content):
        ...
        for css_class in css_classes:
            assert (
                css_class in components_js_content
            ), f"Should include {css_class} CSS class"
    ...
    def test_dashboard_bootstrap_integration(self, components_js_content):
        ...
        assert (
            "bi bi-shield-check" in components_js_content
        ), "Should use Bootstrap icons"
        assert (
            "bi bi-exclamation-triangle-fill" in components_js_content
        ), "Should use error icon"
        assert (
            "bi bi-arrow-clockwise" in components_js_content
        ), "Should use refresh icon"
