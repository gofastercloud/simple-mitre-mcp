...
class TestSmartFormControlsEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_techniques_endpoint_success(self, http_proxy, sample_techniques_data):
        """Test successful techniques endpoint response."""
        # Mock the data loader and cached data
        mock_data_loader = Mock()
        mock_data_loader.get_cached_data.return_value = {"techniques": sample_techniques_data}
        http_proxy.mcp_server.data_loader = mock_data_loader
    ...
    async def test_techniques_endpoint_missing_query(self, http_proxy):
        ...
        response_data = json.loads(response.body)
        assert "error" in response_data
        assert "query parameter" in response_data["error"].lower() and "required" in response_data["error"].lower()
...
class TestSmartFormControlsDataExtraction:
    ...
    def test_extract_groups_invalid_data(self, http_proxy):
        ...
        result = http_proxy._extract_groups_for_dropdown([{"name": "APT29"}])  # Missing ID
        assert result == []
...
class TestSmartFormControlsValidation:
    ...
    def test_input_sanitization(self):
        ...
        def sanitize_input(input_str):
            ...
            sql_patterns = [r"';", r"--", r"/\*", r"\*/", r"DROP\s+TABLE", r"SELECT\s+\*"]
...
class TestSmartFormControlsIntegration:
    ...
    def test_form_controls_performance_metrics(self):
        ...
        assert metrics["autocomplete_response_time"] < 200  # Autocomplete should be fast
