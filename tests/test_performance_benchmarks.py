...
class PerformanceBenchmark:
    ...
    def create_large_stix_dataset(self, num_techniques: int = 1000, 
                                 num_groups: int = 200, 
                                 num_mitigations: int = 500) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Creating synthetic STIX dataset: {num_techniques} techniques, "
        ...
        for i in range(num_techniques):
            ...
            technique = {
                ...
                "description": f"This is a test technique number {i} for performance testing. " * 10,
                "kill_chain_phases": [
                    {
                ...
                "x_mitre_data_sources": [f"Process: Process Creation", f"File: File Creation"],
        ...
        for i in range(num_groups):
            ...
            group = {
                ...
                "modified": "2023-01-01T00:00:00.000Z",
                "name": f"Test Group {i}",
                "description": f"This is a test threat group number {i} for performance testing. " * 10,
        ...
        for i in range(num_mitigations):
            ...
            mitigation = {
                ...
                "description": f"This is a test mitigation number {i} for performance testing. " * 10,
                "external_references": [
                    {
                        "source_name": "mitre-attack",
        ...
        technique_ids = [obj['id'] for obj in objects if obj['type'] == 'attack-pattern']
    ...
    def benchmark_parsing_performance(self, stix_data: Dict[str, Any], 
    ...
    def calculate_performance_statistics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if stix2_successful:
            ...
            entities_extracted = [r['total_entities_extracted'] for r in stix2_successful]
            
            stats['stix2_library_performance'] = {
                'success_rate': len(stix2_successful) / len(results['stix2_library_results']),
                'avg_execution_time': sum(execution_times) / len(execution_times),
                'min_execution_time': min(execution_times),
                'max_execution_time': max(execution_times),
                'avg_memory_delta_mb': sum(memory_deltas) / len(memory_deltas),
                'avg_entities_extracted': sum(entities_extracted) / len(entities_extracted),
                'entities_per_second': sum(entities_extracted) / sum(execution_times) if sum(execution_times) > 0 else 0
            }
        else:
            stats['stix2_library_performance'] = {
                'success_rate': 0,
        ...
        custom_successful = [r for r in results['custom_fallback_results'] if r['success']]
        if custom_successful:
            ...
            entities_extracted = [r['total_entities_extracted'] for r in custom_successful]
            
            stats['custom_fallback_performance'] = {
                'success_rate': len(custom_successful) / len(results['custom_fallback_results']),
                'avg_execution_time': sum(execution_times) / len(execution_times),
                'min_execution_time': min(execution_times),
                'max_execution_time': max(execution_times),
                'avg_memory_delta_mb': sum(memory_deltas) / len(memory_deltas),
        ...
        if 'stix2_library_performance' in stats and 'custom_fallback_performance' in stats:
            stix2_perf = stats['stix2_library_performance']
            custom_perf = stats['custom_fallback_performance']
            
            if stix2_perf.get('avg_execution_time', 0) > 0 and custom_perf.get('avg_execution_time', 0) > 0:
                stats['performance_comparison'] = {
                    'speed_ratio': custom_perf['avg_execution_time'] / stix2_perf['avg_execution_time'],
                    'memory_ratio': custom_perf['avg_memory_delta_mb'] / stix2_perf['avg_memory_delta_mb'] if stix2_perf['avg_memory_delta_mb'] != 0 else 'N/A',
                    'throughput_ratio': stix2_perf['entities_per_second'] / custom_perf['entities_per_second'] if custom_perf['entities_per_second'] > 0 else 'N/A'
                }
...
def small_stix_dataset():
    ...
    return benchmark.create_large_stix_dataset(num_techniques=50, num_groups=10, num_mitigations=25)
...
def medium_stix_dataset():
    ...
    benchmark = PerformanceBenchmark()
    return benchmark.create_large_stix_dataset(num_techniques=200, num_groups=40, num_mitigations=100)
...
def large_stix_dataset():
    ...
    benchmark = PerformanceBenchmark()
    return benchmark.create_large_stix_dataset(num_techniques=1000, num_groups=200, num_mitigations=500)
...
class TestParsingPerformance:
    ...
    def test_small_dataset_performance(self, performance_benchmark, small_stix_dataset):
        ...
        logger.info(f"Data size: {stats['data_characteristics']['data_size_mb']:.2f} MB")
        logger.info(f"Total objects: {stats['data_characteristics']['total_objects']}")
        
        if 'stix2_library_performance' in stats:
            stix2_perf = stats['stix2_library_performance']
            logger.info(f"STIX2 library - Success rate: {stix2_perf.get('success_rate', 0):.2%}")
            logger.info(f"STIX2 library - Avg execution time: {stix2_perf.get('avg_execution_time', 0):.3f}s")
            logger.info(f"STIX2 library - Entities per second: {stix2_perf.get('entities_per_second', 0):.1f}")
        
        if 'custom_fallback_performance' in stats:
            custom_perf = stats['custom_fallback_performance']
            logger.info(f"Custom fallback - Success rate: {custom_perf.get('success_rate', 0):.2%}")
            logger.info(f"Custom fallback - Avg execution time: {custom_perf.get('avg_execution_time', 0):.3f}s")
            logger.info(f"Custom fallback - Entities per second: {custom_perf.get('entities_per_second', 0):.1f}")
        ...
        assert stix2_success or custom_success, "At least one parsing method should succeed"
    
    def test_medium_dataset_performance(self, performance_benchmark, medium_stix_dataset):
        ...
        if 'performance_comparison' in stats:
            ...
            logger.info(f"Performance comparison - Speed ratio: {comparison.get('speed_ratio', 'N/A')}")
            logger.info(f"Performance comparison - Memory ratio: {comparison.get('memory_ratio', 'N/A')}")
            logger.info(f"Performance comparison - Throughput ratio: {comparison.get('throughput_ratio', 'N/A')}")
        
        # Assertions
        assert results['data_size_mb'] > 0.1, "Medium dataset should be at least 0.1 MB"
        assert results['total_objects'] > 100, "Medium dataset should have substantial objects"
    ...
    def test_large_dataset_performance(self, performance_benchmark, large_stix_dataset):
        ...
        entity_types = ['techniques', 'groups', 'mitigations']
        results = performance_benchmark.benchmark_parsing_performance(
            large_stix_dataset, entity_types, iterations=2  # Fewer iterations for large dataset
        )
        ...
        if 'stix2_library_performance' in stats:
            ...
            logger.info(f"  - Avg execution time: {stix2_perf.get('avg_execution_time', 0):.3f}s")
            logger.info(f"  - Memory usage: {stix2_perf.get('avg_memory_delta_mb', 0):.2f} MB")
            logger.info(f"  - Throughput: {stix2_perf.get('entities_per_second', 0):.1f} entities/sec")
            
            # Performance assertions
            assert stix2_perf.get('avg_execution_time', float('inf')) < 30, "Large dataset parsing should complete within 30 seconds"
            assert stix2_perf.get('avg_memory_delta_mb', float('inf')) < 500, "Memory usage should be reasonable (< 500 MB)"
...
class TestMCPToolPerformance:
    ...
    def test_mcp_tool_response_times(self, performance_benchmark):
        ...
        with patch.object(performance_benchmark.data_loader, 'load_data_source') as mock_load:
            ...
            for tool_name, params in mcp_tools:
                ...
                try:
                    ...
                    assert response_time < 5.0, f"{tool_name} response time too slow: {response_time:.3f}s"
            ...
            # Log summary
            avg_response_time = sum(t for t in response_times.values() if t != float('inf')) / len([t for t in response_times.values() if t != float('inf')])
...
class TestMemoryUsageOptimization:
    ...
    def test_memory_usage_patterns(self, performance_benchmark, medium_stix_dataset):
        ...
        _, stix2_memory_stats = performance_benchmark.measure_memory_usage(
            ...
            medium_stix_dataset, entity_types
        )
        ...
        logger.info(f"  - Memory delta: {custom_memory_stats['memory_delta_mb']:.2f} MB")
        logger.info(f"  - Peak traced: {custom_memory_stats['peak_traced_mb']:.2f} MB")
        
        # Memory usage should be reasonable
        assert stix2_memory_stats['memory_delta_mb'] < 200, "STIX2 library memory usage should be reasonable"
        assert custom_memory_stats['memory_delta_mb'] < 200, "Custom fallback memory usage should be reasonable"
        
        # Peak memory should not be excessive
        assert stix2_memory_stats['peak_traced_mb'] < 300, "Peak memory usage should not be excessive"
        assert custom_memory_stats['peak_traced_mb'] < 300, "Peak memory usage should not be excessive"
    ...
    def test_memory_leak_detection(self, performance_benchmark, small_stix_dataset):
        ...
        for i in range(10):
            ...
            if i % 3 == 0:  # Check memory every few iterations
                ...
                assert memory_growth < 100, f"Excessive memory growth detected: {memory_growth:.2f} MB"
        ...
        assert total_growth < 150, f"Potential memory leak detected: {total_growth:.2f} MB growth"
...
class TestRealDataPerformance:
    ...
    def test_real_mitre_attack_performance(self, performance_benchmark):
        ...
        if 'stix2_library_performance' in stats:
            ...
            assert stix2_perf.get('success_rate', 0) >= 0.95, "Success rate should be at least 95%"
            assert stix2_perf.get('avg_execution_time', float('inf')) < 15, "Real data parsing should complete within 15 seconds"
        
        # Ensure we can extract a reasonable number of entities
        successful_results = [r for r in results['stix2_library_results'] if r['success']]
        if successful_results:
            avg_entities = sum(r['total_entities_extracted'] for r in successful_results) / len(successful_results)
            logger.info(f"Average entities extracted: {avg_entities:.0f}")
            assert avg_entities > 1000, "Should extract substantial number of entities from real data"
...
if __name__ == "__main__":
    ...
    small_results = benchmark.benchmark_parsing_performance(small_data, ['techniques', 'groups', 'mitigations'])
    small_stats = benchmark.calculate_performance_statistics(small_results)
    
    print("Running medium dataset benchmark...")
    medium_results = benchmark.benchmark_parsing_performance(medium_data, ['techniques', 'groups', 'mitigations'])
    medium_stats = benchmark.calculate_performance_statistics(medium_results)
    
    # Print summary
    print("\n=== PERFORMANCE BENCHMARK SUMMARY ===")
    print(f"Small dataset ({small_stats['data_characteristics']['data_size_mb']:.2f} MB):")
    if 'stix2_library_performance' in small_stats:
        print(f"  STIX2 library: {small_stats['stix2_library_performance']['avg_execution_time']:.3f}s")
    if 'custom_fallback_performance' in small_stats:
        print(f"  Custom fallback: {small_stats['custom_fallback_performance']['avg_execution_time']:.3f}s")
    
    print(f"Medium dataset ({medium_stats['data_characteristics']['data_size_mb']:.2f} MB):")
    if 'stix2_library_performance' in medium_stats:
        print(f"  STIX2 library: {medium_stats['stix2_library_performance']['avg_execution_time']:.3f}s")
    if 'custom_fallback_performance' in medium_stats:
        print(f"  Custom fallback: {medium_stats['custom_fallback_performance']['avg_execution_time']:.3f}s")