...
class TestTask14Integration:
    ...
    async def test_search_attack_tool(self):
        ...
        with patch.object(
            ...
            for query in test_queries:
                ...
                assert isinstance(
                    result, tuple
                ), f"Expected tuple result for query '{query}'"
                assert (
                    len(result) == 2
                ), f"Expected tuple with 2 elements for query '{query}'"
                ...
                ), f"Expected non-empty results for query '{query}'"

                # Verify content structure
                content = search_results[0]
                assert hasattr(
                    content, "text"
                ), f"Expected text content for query '{query}'"
                assert isinstance(
                    content.text, str
                ), f"Expected string text for query '{query}'"
                ...
                if query == "T1055":
                    assert (
                        "T1055" in content.text
                    ), "T1055 should be found in technique search"
                    assert (
                        "Process Injection" in content.text
                    ), "Technique name should be included"
                elif query == "APT29":
                    assert (
                        "G0016" in content.text
                    ), "Group ID should be found in group search"
                elif query == "nonexistent_term":
                    assert (
                        "No results found" in content.text
    ...
    async def test_get_technique_tool(self):
        ...
        with patch.object(
            ...
            for technique_id in valid_technique_ids:
                ...
                assert isinstance(
                    result, tuple
                ), f"Expected tuple result for technique {technique_id}"
                assert (
                    len(result) == 2
                ), f"Expected tuple with 2 elements for technique {technique_id}"

                technique_info, metadata = result
                assert isinstance(
                    technique_info, list
                ), f"Expected list result for technique {technique_id}"
                assert (
                    len(technique_info) > 0
                ), f"Expected non-empty results for technique {technique_id}"

                content = technique_info[0]
                assert hasattr(
                    content, "text"
                ), f"Expected text content for technique {technique_id}"

                # Verify technique details are included
                text = content.text
                assert (
                    technique_id in text
                ), f"Technique ID should be in response for {technique_id}"
                assert (
                    "TECHNIQUE DETAILS" in text
                ), f"Should include technique details header for {technique_id}"
                assert (
                    "Description:" in text
                ), f"Should include description for {technique_id}"
                assert (
                    "Associated Tactics" in text
                ), f"Should include tactics for {technique_id}"
                assert (
                    "Platforms" in text
                ), f"Should include platforms for {technique_id}"
                assert (
                    "Mitigations" in text
                ), f"Should include mitigations for {technique_id}"
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for invalid technique"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for invalid technique"

            error_info, metadata = result
            assert (
                "not found" in error_info[0].text
    ...
    async def test_list_tactics_tool(self):
        ...
        with patch.object(
            ...
            assert isinstance(
                tactics_list, list
            ), "Expected list result from list_tactics"
    ...
    async def test_get_group_techniques_tool(self):
        ...
        with patch.object(
            ...
            for group_id in valid_group_ids:
                ...
                result = await mcp_server.call_tool(
                    "get_group_techniques", {"group_id": group_id}
                )

                # MCP server returns tuple of (result_list, metadata_dict)
                assert isinstance(
                    result, tuple
                ), f"Expected tuple result for group {group_id}"
                assert (
                    len(result) == 2
                ), f"Expected tuple with 2 elements for group {group_id}"

                group_info, metadata = result
                assert isinstance(
                    group_info, list
                ), f"Expected list result for group {group_id}"
                assert (
                    len(group_info) > 0
                ), f"Expected non-empty results for group {group_id}"

                content = group_info[0]
                text = content.text

                assert (
                    "GROUP TECHNIQUES" in text
                ), f"Should include group techniques header for {group_id}"
                assert (
                    group_id in text
                ), f"Group ID should be in response for {group_id}"
                assert (
                    "Techniques Used" in text
                ), f"Should include techniques section for {group_id}"
    ...
    async def test_get_technique_mitigations_tool(self):
        ...
        with patch.object(
            ...
            for technique_id in test_techniques:
                ...
                assert isinstance(
                    mitigation_info, list
                ), f"Expected list result for technique {technique_id}"
                assert (
                    len(mitigation_info) > 0
                ), f"Expected non-empty results for technique {technique_id}"
                ...
                assert (
                    "TECHNIQUE MITIGATIONS" in text
                ), f"Should include mitigations header for {technique_id}"
                assert (
                    technique_id in text
                ), f"Technique ID should be in response for {technique_id}"
                assert (
                    "Mitigations" in text
                ), f"Should include mitigations section for {technique_id}"
                ...
                logger.info(
                    f"Technique {technique_id} mitigations verified successfully"
                )
    ...
    async def test_build_attack_path_tool(self):
        ...
        with patch.object(
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for build_attack_path"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for build_attack_path"

            path_info, metadata = result
            assert isinstance(
                path_info, list
            ), "Expected list result from build_attack_path"
            ...
            assert (
                "ATTACK PATH CONSTRUCTION" in text
            ), "Should include attack path header"
            ...
            result = await mcp_server.call_tool(
                "build_attack_path",
                {"start_tactic": "TA0001", "end_tactic": "TA0004", "group_id": "G0016"},
            )

            # MCP server returns tuple of (result_list, metadata_dict)
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for filtered build_attack_path"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for filtered build_attack_path"
            ...
            assert (
                "Group Filter: G0016" in text
            ), "Should include group filter information"
    ...
    async def test_analyze_coverage_gaps_tool(self):
        ...
        with patch.object(
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for analyze_coverage_gaps"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for analyze_coverage_gaps"

            coverage_info, metadata = result
            assert isinstance(
                coverage_info, list
            ), "Expected list result from analyze_coverage_gaps"
            ...
            assert (
                "COVERAGE GAP ANALYSIS" in text
            ), "Should include coverage analysis header"
            ...
            logger.info(
                "Coverage gap analysis with threat groups verified successfully"
            )

            # Test coverage analysis with specific techniques
            result = await mcp_server.call_tool(
                "analyze_coverage_gaps", {"technique_list": ["T1055", "T1059"]}
            )

            # MCP server returns tuple of (result_list, metadata_dict)
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for technique coverage analysis"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for technique coverage analysis"
    ...
    async def test_detect_technique_relationships_tool(self):
        ...
        with (
            patch.object(
                self.data_loader, "get_cached_data", return_value=enhanced_data
            ),
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for detect_technique_relationships"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for detect_technique_relationships"

            relationship_info, metadata = result
            assert isinstance(
                relationship_info, list
            ), "Expected list result from detect_technique_relationships"
            ...
            assert (
                "TECHNIQUE RELATIONSHIP ANALYSIS" in text
            ), "Should include relationship analysis header"
            assert (
                "Primary Technique: T1055" in text
            ), "Should include primary technique"
    ...
    async def test_all_tools_error_handling(self):
        ...
        for tool_name, params in tools_to_test:
            ...
            assert (
                len(tool_result) > 0
            ), f"Should return non-empty result for {tool_name}"

            content = tool_result[0]
            text = content.text

            assert (
                "Error: Data loader not available" in text
            ), f"Should indicate data loader error for {tool_name}"
    ...
    async def test_tools_with_empty_data(self):
        ...
        with patch.object(self.data_loader, "get_cached_data", return_value=empty_data):
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for search with empty data"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for search with empty data"

            search_result, metadata = result
            assert (
                "No results found" in search_result[0].text
            ...
            assert isinstance(
                result, tuple
            ), f"Expected tuple result for tactics with empty data"
            assert (
                len(result) == 2
            ), f"Expected tuple with 2 elements for tactics with empty data"

            tactics_result, metadata = result
            assert (
                "No tactics found" in tactics_result[0].text
            ), "Should indicate no tactics"
    ...
    async def test_mcp_server_integration_end_to_end(self):
        ...
        with patch.object(
            ...
            workflow_steps = [
                ...
                (
                    "build_attack_path",
                    {"start_tactic": "TA0001", "end_tactic": "TA0005"},
                ),
            ...
            for step_num, (tool_name, params) in enumerate(workflow_steps, 1):
                ...
                assert isinstance(
                    result, tuple
                ), f"Expected tuple result for workflow step {step_num} ({tool_name})"
                assert (
                    len(result) == 2
                ), f"Expected tuple with 2 elements for workflow step {step_num} ({tool_name})"

                tool_result, metadata = result
                assert isinstance(
                    tool_result, list
                ), f"Step {step_num} should return list"
                assert (
                    len(tool_result) > 0
                ), f"Step {step_num} should return non-empty result"

                content = tool_result[0]
                assert hasattr(
                    content, "text"
                ), f"Step {step_num} should return text content"
                assert (
                    len(content.text) > 0
                ), f"Step {step_num} should return non-empty text"
                ...
                logger.info(
                    f"Step {step_num} completed successfully - {len(content.text)} characters returned"
                )
            ...
            assert len(successful_steps) == len(
                workflow_steps
            ), "All workflow steps should succeed"

            total_output = sum(r["result_length"] for r in workflow_results)
            logger.info(
                f"Complete workflow generated {total_output} characters of output across {len(workflow_steps)} steps"
            )
            ...
            for result in workflow_results:
                logger.info(
                    f"  Step {result['step']}: {result['tool']} - {result['result_length']} chars"
                )
...
class TestHTTPProxyIntegration:
    ...
    def test_http_proxy_creation(self):
        ...
        with patch.object(
            ...
            try:
                ...
                assert hasattr(
                    proxy, "mcp_server"
                ), "HTTP proxy should have mcp_server attribute"

                # Verify routes are configured
                if hasattr(proxy.app, "router"):
                    routes = [
                        route.resource.canonical for route in proxy.app.router.routes()
                    ]
                ...
                logger.warning(
                    "HTTP proxy module not available - skipping HTTP proxy tests"
                )
    ...
    def test_web_interface_compatibility(self):
        ...
        with patch.object(
            ...
            ), "MCP server should have call_tool method"
            ...
            async def test_web_calls():
                ...
                assert isinstance(
                    result, tuple
                ), f"Expected tuple result for web interface test"
                assert (
                    len(result) == 2
                ), f"Expected tuple with 2 elements for web interface test"
