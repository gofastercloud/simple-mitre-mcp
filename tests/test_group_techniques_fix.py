...
class TestGroupTechniquesFix:
    ...
    def test_apt29_specific_fix(self, loaded_data):
        ...
        assert len(s_prefixed) == 0, f"APT29 still contains S-prefixed IDs: {s_prefixed}"
        assert len(t_prefixed) == len(techniques), "Not all APT29 techniques are T-prefixed"
        
        # The count should be reasonable (less than the original 115 that included software)
        assert len(techniques) < 100, f"APT29 technique count seems too high: {len(techniques)}"
        assert len(techniques) > 30, f"APT29 technique count seems too low: {len(techniques)}"
    ...
    def test_no_name_not_found_in_resolved_techniques(self, loaded_data):
        ...
        if apt29:
            ...
            for tech_id in group_techniques:
                ...
                assert tech_info is not None, f"Technique {tech_id} not found in techniques list"
                
                tech_name = tech_info.get("name", "")
                assert tech_name != "(Name not found)", f"Technique {tech_id} has '(Name not found)'"
                assert tech_name != "", f"Technique {tech_id} has empty name"
                assert "not available" not in tech_name.lower(), f"Technique {tech_id} has placeholder name: {tech_name}"