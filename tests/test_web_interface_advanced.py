...
class TestWebInterfaceAdvanced:
    ...
    def test_web_explorer_html_contains_advanced_section(self):
        ...
        assert 'Attack Path Analysis' in html_content, "Attack Path Analysis demo not found"
        assert 'Coverage Gap Analysis' in html_content, "Coverage Gap Analysis demo not found"
        assert 'Relationship Discovery' in html_content, "Relationship Discovery demo not found"

        # Check for advanced tool demos
        assert 'build_attack_path' in html_content, "build_attack_path tool not found"
        assert 'analyze_coverage_gaps' in html_content, "analyze_coverage_gaps tool not found"
        assert 'detect_technique_relationships' in html_content, "detect_technique_relationships tool not found"
    ...
    def test_web_explorer_html_contains_advanced_css(self):
        ...
        assert '.custom-input-section' in html_content, "Custom input section CSS not found"
    ...
    def test_web_explorer_html_javascript_functions(self):
        ...
        for func_name in required_functions:
            assert f'function {func_name}' in html_content or f'{func_name}(' in html_content, f"JavaScript function '{func_name}' not found"

        # Check for demo configurations
        assert 'attack_path:' in html_content, "Attack path demo configuration not found"
        assert 'coverage_gaps:' in html_content, "Coverage gaps demo configuration not found"
        assert 'technique_relationships:' in html_content, "Technique relationships demo configuration not found"
    ...
    def test_web_explorer_html_form_configurations(self):
        ...
        assert 'analyze_coverage_gaps' in html_content, "analyze_coverage_gaps option not found"
        assert 'detect_technique_relationships' in html_content, "detect_technique_relationships option not found"
    ...
    def test_http_proxy_contains_advanced_tools(self):
        ...
        assert 'multi-stage attack paths' in proxy_content, "Attack path description not found"
        assert 'coverage gaps' in proxy_content, "Coverage gaps description not found"
        assert 'STIX relationships' in proxy_content, "STIX relationships description not found"
    ...
    def test_http_proxy_tool_schemas(self):
        ...
        assert '"items": {"type": "string"}' in proxy_content, "Array items type not found"

    def test_start_explorer_script_compatibility(self):
        """Test that start_explorer.py is compatible with the new interface."""
        # Use relative path that works in both local and CI environments
        script_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'start_explorer.py')
        
        with open(script_path, 'r') as f:
            script_content = f.read()

        # Check for async compatibility
        assert 'async def' in script_content, "Async functions not found in start_explorer.py"
        assert 'create_http_proxy_server' in script_content, "create_http_proxy_server import not found"
    ...
    def test_web_interface_tool_count_consistency(self):
        ...
        assert '8 Tools Available' in html_content or '8 (5 basic + 3 advanced' in html_content, "Tool count not found in web interface"

        # Count tools in HTTP proxy
        tool_count = proxy_content.count('"name":')
        assert tool_count >= 8, f"Expected at least 8 tools in HTTP proxy, found {tool_count}"
