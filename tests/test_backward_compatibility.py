...
class TestBackwardCompatibility:
    ...
    def test_mitigation_extraction_output_identical(self):
        ...
        # Parse with new STIX2 library method
        new_result = self.parser._parse_with_stix2_library(
            self.sample_stix_data, ["mitigations"]
        )
    ...
    async def test_all_mcp_tools_work_with_refactored_parser(self):
        ...
        for tool_name, params in tools_to_test:
            ...
            if tool_name == "search_attack":
                assert (
                    "T1055" in result[0].text or "Process Injection" in result[0].text
                )
    ...
    def test_edge_case_malformed_data_handling(self):
        ...
        for malformed_data in malformed_cases:
            ...
            try:
                ...
                assert isinstance(
                    e, (STIXError, InvalidValueError, MissingPropertiesError)
                )
    ...
    def test_performance_regression_check(self):
        ...
        assert (
            parsing_time < 5.0
        ), f"Parsing took {parsing_time:.2f} seconds, which may indicate performance regression"
    ...
    def test_empty_and_null_field_handling(self):
        ...
        minimal_technique = stix2.AttackPattern(
            ...
            external_references=[
                stix2.ExternalReference(source_name="mitre-attack", external_id="T8888")
            ],
            allow_custom=True,
        )
    ...
    async def test_real_mitre_attack_data_integration(self):
        ...
        assert (
            len(result["techniques"]) > 100
        ), f"Expected >100 techniques, got {len(result['techniques'])}"
        assert (
            len(result["groups"]) > 50
        ), f"Expected >50 groups, got {len(result['groups'])}"
        assert (
            len(result["tactics"]) > 10
        ), f"Expected >10 tactics, got {len(result['tactics'])}"
        assert (
            len(result["mitigations"]) > 30
        ), f"Expected >30 mitigations, got {len(result['mitigations'])}"
        ...
        for technique in result["techniques"][:5]:  # Check first 5 techniques
            ...
            assert "mitigations" in technique and isinstance(
                technique["mitigations"], list
            )
        ...
        for mitigation in result["mitigations"][:5]:  # Check first 5 mitigations
            ...
            assert mitigation["id"].startswith(
                ("M", "T")
            )  # Allow both M and T prefixes
            assert "name" in mitigation and len(mitigation["name"]) > 0
            assert "description" in mitigation
            assert "techniques" in mitigation and isinstance(
        ...
        with patch.object(data_loader, "download_data") as mock_download:
            ...
            with patch.object(
                ...
                assert all(
                    entity_type in processed_data for entity_type in entity_types
                )
                ...
                assert (
                    "T1055" in search_result[0].text
                    or "process" in search_result[0].text.lower()
                )
    ...
    def test_stix2_library_error_handling_comprehensive(self):
        ...
        for i, error_case in enumerate(error_test_cases):
            try:
                ...
                assert isinstance(
                    e,
                    (STIXError, InvalidValueError, MissingPropertiesError, ValueError),
                )
                print(
                    f"Error test case {i + 1} raised expected exception: {type(e).__name__}"
                )
    ...
    def test_large_dataset_performance_and_memory(self):
        ...
        for i in range(100):
            technique = stix2.AttackPattern(
                ...
                description=f"This is a test technique {i} with a longer description to simulate real data size. "
                * 5,
                external_references=[
                    stix2.ExternalReference(
                        source_name="mitre-attack",
        ...
        for i in range(50):
            group = stix2.IntrusionSet(
                ...
                description=f"This is a test group {i} with detailed attribution information. "
                * 3,
                aliases=[
                    f"Large Test Group {i:03d}",
                    f"LTG{i:03d}",
                    f"Group{i:03d}",
                    f"TestGroup{i:03d}",
                ],
        ...
        assert (
            parsing_time < 10.0
        ), f"Large dataset parsing took {parsing_time:.2f} seconds, which may indicate performance issues"
