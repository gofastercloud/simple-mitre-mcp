...
class PerformanceOptimizer:
    ...
    def profile_parsing_methods(
        ...
        stix2_profile = self._profile_function(
            self.benchmark.parser._parse_with_stix2_library, stix_data, entity_types
        )
        ...
        custom_profile = self._profile_function(
            self.benchmark.parser._parse_with_custom_logic, stix_data, entity_types
        )
        results["custom_fallback_profile"] = custom_profile

        # Analyze bottlenecks
        results["bottlenecks"] = self._identify_bottlenecks(
            stix2_profile, custom_profile
        )
    ...
    def _identify_bottlenecks(
        self, stix2_profile: Dict[str, Any], custom_profile: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
                    ...
                    bottlenecks.append(
                        {
                            ...
                            "severity": (
                                "high" if func["cumulative_time"] > 0.5 else "medium"
                            ),
                        }
                    )
        ...
        if (
            stix2_profile["success"]
            and custom_profile["success"]
            and stix2_profile["execution_time"] > custom_profile["execution_time"] * 1.5
        ):
            bottlenecks.append(
                {
                    ...
                    "slowdown_factor": stix2_profile["execution_time"]
                    / custom_profile["execution_time"],
                    "severity": "high",
                }
            )
    ...
    def _generate_optimization_recommendations(
        ...
        custom_bottlenecks = [
            b for b in bottlenecks if b.get("type") == "custom_fallback"
        ]
        regression_bottlenecks = [
            b for b in bottlenecks if b.get("type") == "performance_regression"
        ]
        ...
        if not recommendations:
            recommendations.append(
                "No significant performance bottlenecks detected. Current implementation is well-optimized."
            )
    ...
    def run_comprehensive_optimization_analysis(self) -> Dict[str, Any]:
        ...
        for dataset_name, dataset in datasets.items():
            ...
            benchmark_stats = self.benchmark.calculate_performance_statistics(
                benchmark_results
            )
            ...
            analysis_results[dataset_name] = {
                "dataset_size_mb": len(json.dumps(dataset).encode("utf-8"))
                / 1024
                / 1024,
    ...
    def _generate_optimization_summary(
        ...
        for results in analysis_results.values():
            ...
            summary["high_severity_bottlenecks"] += len(
                [b for b in bottlenecks if b.get("severity") == "high"]
            )
            summary["performance_regressions"] += len(
                [b for b in bottlenecks if b.get("type") == "performance_regression"]
            )
        ...
        if custom_times:
            summary["average_custom_performance"] = sum(custom_times) / len(
                custom_times
            )
...
def main():
    ...
    try:
        ...
        print(
            f"Average STIX2 library performance: {summary['average_stix2_performance']:.3f}s"
        )
        print(
            f"Average custom fallback performance: {summary['average_custom_performance']:.3f}s"
        )
        ...
        for dataset_name, dataset_results in results["analysis_results"].items():
            ...
            if "stix2_library_performance" in benchmark:
                ...
                print(
                    f"  STIX2 library: {stix2_perf.get('avg_execution_time', 0):.3f}s"
                )

            if "custom_fallback_performance" in benchmark:
                custom_perf = benchmark["custom_fallback_performance"]
                print(
                    f"  Custom fallback: {custom_perf.get('avg_execution_time', 0):.3f}s"
                )
