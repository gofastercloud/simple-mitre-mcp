...
class TestComprehensiveMCPIntegration:
    ...
    @pytest.mark.asyncio
    async def test_all_8_mcp_tools_registered_and_functional(self, mcp_server_with_refactored_parser):
    ...
    @pytest.mark.asyncio
    async def test_basic_analysis_tools_with_refactored_parser(self, mcp_server_with_refactored_parser):
        ...
        group_result, _ = await mcp_server.call_tool('get_group_techniques', {'group_id': 'G0016'})
        ...
        mitigations_result, _ = await mcp_server.call_tool('get_technique_mitigations', {'technique_id': 'T1078'})
    ...
    @pytest.mark.asyncio
    async def test_advanced_threat_modeling_tools_with_refactored_parser(self, mcp_server_with_refactored_parser):
        ...
        try:
            attack_path_result, _ = await mcp_server.call_tool('build_attack_path', {
        ...
        try:
            coverage_gaps_result, _ = await mcp_server.call_tool('analyze_coverage_gaps', {
        ...
        try:
            relationships_result, _ = await mcp_server.call_tool('detect_technique_relationships', {
                'technique_id': 'T1055'
            })
    ...
    @pytest.mark.asyncio
    async def test_error_handling_consistency_across_all_tools(self, mcp_server_with_refactored_parser):
        ...
        error_test_cases = [
            ...
            ('build_attack_path', {'start_tactic': 'INVALID_TACTIC', 'end_tactic': 'TA0001'}),
        ...
        for tool_name, invalid_params in error_test_cases:
            ...
            assert any(keyword in result[0].text.lower() for keyword in ['not found', 'invalid', 'error', 'no'])

    @pytest.mark.asyncio
    async def test_data_consistency_across_all_tools_with_refactored_parser(self, mcp_server_with_refactored_parser):
        ...
        technique_result, _ = await mcp_server.call_tool('get_technique', {'technique_id': technique_id})
        technique_text = technique_result[0].text

        # Search for the same technique
        search_result, _ = await mcp_server.call_tool('search_attack', {'query': technique_id})
        search_text = search_result[0].text

        # Get mitigations for the technique
        mitigations_result, _ = await mcp_server.call_tool('get_technique_mitigations', {'technique_id': technique_id})
    ...
    @pytest.mark.asyncio
    async def test_advanced_tool_parameter_validation_with_refactored_parser(self, mcp_server_with_refactored_parser):
        ...
        assert coverage_tool.inputSchema['properties']['threat_groups']['type'] == 'array'
    ...
    @pytest.mark.asyncio
    async def test_performance_with_comprehensive_data_and_refactored_parser(self, mcp_server_with_refactored_parser):
        ...
        for tool_name, params in performance_tests:
            ...
            assert execution_time < 1.0, f"Tool {tool_name} took {execution_time:.2f}s, which may indicate performance issues"
        ...
        assert total_time < 5.0, f"Total execution time {total_time:.2f}s may indicate performance regression"
    ...
    def test_refactored_parser_maintains_data_structure_integrity(self):
        ...
        expected_technique_fields = ['id', 'name', 'description', 'platforms', 'tactics', 'mitigations']
        for field in expected_technique_fields:
            assert field in parsed_technique, f"Missing field '{field}' in parsed technique"
