...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    for source_name, source_config in config["data_sources"].items():
        if "url" not in source_config:
            raise ValueError(
                f"Data source '{source_name}' missing required 'url' field"
            )
        if "format" not in source_config:
            raise ValueError(
                f"Data source '{source_name}' missing required 'format' field"
            )
        if "entity_types" not in source_config:
            raise ValueError(
                f"Data source '{source_name}' missing required 'entity_types' field"
            )
    ...
    for schema_name, schema_config in config["entity_schemas"].items():
        if "id_field" not in schema_config:
            raise ValueError(
                f"Entity schema '{schema_name}' missing required 'id_field'"
            )
        if "name_field" not in schema_config:
            raise ValueError(
                f"Entity schema '{schema_name}' missing required 'name_field'"
            )
        if "required_fields" not in schema_config:
            raise ValueError(
                f"Entity schema '{schema_name}' missing required 'required_fields'"
            )
    ...
    def load_data_sources_config(self) -> Dict[str, Any]:
        ...
        try:
            with open(data_sources_path, "r") as f:
                data_sources_config = yaml.safe_load(f)
                logger.info(
                    f"Loaded {len(data_sources_config.get('data_sources', {}))} data sources"
                )
