...
def create_mcp_server(data_loader=None):
    ...
    async def search_attack(query: str) -> List[TextContent]:
        ...
        try:
            ...
            # Format results
            result_text = ff"Search results for '{query}' ({len(search_results)} matches):
...
"
            ...
            for result in search_results:
                ...
                result_text += ff"[{entity_type}] {entity_id}: {entity_name}
"
                result_text += ff"  Match: {match_reason}
"
                ...
                if 'description' in result and result['description']:
                    ...
                    result_text += ff"  Description: {desc_preview}
"
...
"
            result_text += ff"ID: {technique.get('id', 'N/A')}
"
            result_text += ff"Name: {technique.get('name', 'N/A')}

"

            # Description
            description = technique.get('description', 'No description available')
            result_text += ff"Description:
{description}
...
"
            ...
            if tactics:
                result_text += ff"Associated Tactics ({len(tactics)}):
"
...
"
            ...
            if platforms:
                result_text += ff"Platforms ({len(platforms)}):
"
...
"
            ...
            if mitigations:
                result_text += ff"Mitigations ({len(mitigations)}):
"
...
"
            ...
            if technique.get('data_sources'):
                result_text += ff"Data Sources: {', '.join(technique['data_sources'])}
"
...
"
            result_text += ff"Total tactics: {len(sorted_tactics)}
...
"
            ...
            for tactic in sorted_tactics:
                ...
                result_text += ff"ID: {tactic_id}
"
                result_text += ff"Name: {tactic_name}
"
                result_text += ff"Description: {tactic_description}
"
                result_text += ff"{'-' * 50}
...
"
            result_text += ff"Group ID: {group.get('id', 'N/A')}
"
            result_text += ff"Group Name: {group.get('name', 'N/A')}
"

            # Add aliases if available
            aliases = group.get('aliases', [])
            if aliases:
                result_text += ff"Aliases: {', '.join(aliases)}
"
...
"
            ...
            result_text += ff"Techniques Used ({len(group_techniques)}):
"
            result_text += ff"{'-' * 40}
...
"
            ...
            for i, tech in enumerate(technique_details, 1):
                result_text += ff"{i}. {tech['id']}: {tech['name']}
"
                ...
                if len(desc) > 150:
                    desc = desc[:150] + "..."
                result_text += ff"   Description: {desc}
"
                ...
                if tech['tactics']:
                    ...
                    result_text += ff"   Tactics: {', '.join(tactic_names)}
"

                # Add platforms if available
                if tech['platforms']:
                    result_text += ff"   Platforms: {', '.join(tech['platforms'])}
"
...
"
            result_text += ff"Technique ID: {technique.get('id', 'N/A')}
"
            result_text += ff"Technique Name: {technique.get('name', 'N/A')}
...
"
            ...
            result_text += ff"Description: {description}

"

            result_text += ff"Mitigations ({len(technique_mitigations)}):
"
            result_text += ff"{'-' * 40}
...
"
            ...
            for i, mitigation in enumerate(mitigation_details, 1):
                result_text += ff"{i}. {mitigation['id']}: {mitigation['name']}
"
...
"
            ...
            if threat_groups:
                ...
                result_text += ff"Threat Groups Analyzed: {', '.join(group_names)}
"

            if technique_list:
                result_text += ff"Specific Techniques: {', '.join(technique_list)}
"

            if exclude_mitigations:
                result_text += ff"Excluded Mitigations: {', '.join(exclude_mitigations)}
"

            result_text += ff"Total Techniques Analyzed: {total_techniques}
...
"
            result_text += "==================
"
            result_text += ff"Techniques with Available Mitigations: {techniques_with_mitigations} ({coverage_percentage:.1f}%)
"
            result_text += ff"Techniques with Coverage Gaps: {techniques_without_mitigations} ({100-coverage_percentage:.1f}%)
"
            result_text += ff"Total Available Mitigations: {len(all_available_mitigations)}
"

            if excluded_mitigations_found:
                result_text += ff"Excluded Mitigations Found: {len(excluded_mitigations_found)}
"
...
"
                result_text += "====================
"
                result_text += ff"Techniques Requiring Attention ({len(gaps_found)}):

"

                for i, analysis in enumerate(gaps_found[:20], 1):  # Limit to 20 for readability
                    result_text += ff"{i}. {analysis['technique_id']} - {analysis['technique_name']}
"
                    result_text += ff"   Status: {analysis['coverage_status']}
"
                    result_text += ff"   Tactics: {', '.join(analysis['tactics'])}
"
                    result_text += ff"   Platforms: {', '.join(analysis['platforms']) if analysis['platforms'] else 'Not specified'}
"

                    if analysis['coverage_status'] == 'GAP':
                        result_text += ff"   Issue: All {analysis['total_mitigations']} mitigations are excluded
"
