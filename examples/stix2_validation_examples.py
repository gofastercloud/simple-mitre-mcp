...
def example_relationship_validation():
    ...
    try:
        ...
        if isinstance(relationship, Relationship):
            ...
            print(
                f"    ✓ Source ref format: {'valid' if relationship.source_ref.startswith('intrusion-set--') else 'invalid'}"
            )
            print(
                f"    ✓ Target ref format: {'valid' if relationship.target_ref.startswith('attack-pattern--') else 'invalid'}"
            )
...
def example_mitre_id_validation():
    ...
    for test_id in test_ids:
        ...
        test_data = {
            ...
            "external_references": [
                {"source_name": "mitre-attack", "external_id": test_id}
            ],
        }
        ...
        try:
            ...
            print(
                f"  {status} {test_id}: {'Valid' if is_valid else 'Invalid'} format - STIX object created"
            )
